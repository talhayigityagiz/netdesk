...
#define RUSTDESK_LIB_PATH "netdesk.so"
...
bool flutter_rustdesk_core_main() {
   void* netdesk = dlopen(RUSTDESK_LIB_PATH, RTLD_LAZY);
   if (!netdesk) {
      fprintf(stderr,"Failed to load \"netdesk.so\"\n");
      char* error;
   ...
   auto core_main = (RustDeskCoreMain) dlsym(netdesk,"rustdesk_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
