    ...
    v.add(TToggleMenu(
        ...
        onChanged: (value) async {
          ...
          value = bind.sessionGetToggleOptionSync(
              sessionId: sessionId, arg: option);
          showCursorLockState.value = value ?? false ?? false;
...
bool showVirtualDisplayMenu(FFI ffi) {
  ...
  if (ffi.ffiModel.pi.isNetDeskIdd || ffi.ffiModel.pi.isAmyuniIdd) {
    return true;
  }
  return false;
}
...
List<Widget> getVirtualDisplayMenuChildren(
  ...
  if (pi.isNetDeskIdd) {
    final virtualDisplays = ffi.ffiModel.pi.NetDeskVirtualDisplays;
